...
public final class XmlStream {
    ...
    public static <T> T fromXML(InputStream content, Class<T> clazz) {
        ...
        try {
            ...
            if (rootElement == null || rootElement.name().equals(XmlRootElement.class.getMethod("name").getDefaultValue().toString())) {
            ...
            throw new RuntimeException("Could not unmarshaller class [" + clazz + "]: " + ex.getMessage(), ex);
    ...
    public static Map<String, String> xml2map(String content) {
        ...
        try {
            ...
            XMLStreamReader xr = xmlInputFactory.createXMLStreamReader(sr);
            while (true) {
                int event = xr.next();
    ...
    public static <T> void toXML(T t, OutputStream os) {
        ...
        try {
            ...
            throw new RuntimeException("Could not marshal class [" + clazz + "]: " + ex.getMessage(), ex);
        } catch (NoSuchMethodException ex) {
            throw new RuntimeException("Could not marshaller class [" + clazz + "]: " + ex.getMessage(), ex);
        } finally {
            if (os != null) {
                try {
                    os.close();
